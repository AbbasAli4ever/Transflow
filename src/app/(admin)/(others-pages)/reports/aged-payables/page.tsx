...
function computeAgingBuckets(supplier: PayableSupplier): AgingBuckets {
  ...
  const docs = supplier.openDocuments ?? [];
  ...
  // Fallback if no openDocuments
  if (docs.length === 0) {
    const outstanding =
      supplier.netOutstanding ?? supplier.totalOutstanding ?? supplier.balance ?? 0;
...
export default function AgedPayablesPage() {
  ...
  const [report, setReport] = useState<PendingPayablesReport | null>(null);
  ...
  async function handleGenerate() {
    ...
    try {
      const data = await getPendingPayables(asOfDate);
      setReport(data);
    } catch (err) {
  ...
  // Normalize: API may return payables[] or suppliers[]
  const rows: PayableSupplier[] = report
    ? (report.payables ?? report.suppliers ?? [])
    : [];
  ...
  function navigateToSupplier(supplierId: string) {
    ...
    <div className="w-full max-w-full mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Track what you owe suppliers and how overdue balances are.
        </p>
      </div>
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3">
                          <button
                            onClick={() => navigateToSupplier(row.supplierId)}
          ...
          {rowsWithBuckets.length > 0 && (
            <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-700">
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Click supplier names to open Supplier Open Documents.
              </p>
            </div>
          )}
        </div>
