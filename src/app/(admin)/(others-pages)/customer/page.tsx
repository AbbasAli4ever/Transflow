...
function CustomerDrawer({
  ...
  onClose: () => void;
  onSaved: () => void;
  initial?: ApiCustomer | null;
}) {
  const fromInitial = (c: ApiCustomer): DrawerForm => ({
    name: c.name,
    phone: c.phone ?? "",
    address: c.address ?? "",
    notes: c.notes ?? "",
...
function ChangeStatusModal({
  ...
  const handleConfirm = async () => {
    ...
    setSaving(true);
    try {
      await changeCustomerStatus(customer.id, newStatus, reason.trim() || undefined);
      onSaved();
      onClose();
    } catch (err) {
      const apiErr = err as ApiError;
  ...
  return (
    <Modal isOpen={!!customer} onClose={onClose} className="max-w-sm mx-4" showCloseButton={false}>
      <div className="p-6">
...
export default function CustomersPage() {
  ...
  const [statusTarget, setStatusTarget] = useState<ApiCustomer | null>(null);
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      ...
      if (isRateLimitError(err)) {
        ...
        const apiErr = err as ApiError;
        setError(apiErr.message ?? "Failed to load customers.");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="mx-auto w-full max-w-full">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-50 dark:bg-brand-500/10">
            <HiOutlineUserGroup size={22} className="text-brand-500" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Customers List</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Browse and manage all customers.
            </p>
          </div>
        </div>
      ...
      {/* Error banner */}
      {rateLimited && (
        <RateLimitBanner onRetry={() => void fetchCustomers()} />
      )}
      {error && !rateLimited && (
        <div className="mb-4 flex items-start gap-2 rounded-xl bg-error-50 px-4 py-3 text-sm text-error-600 dark:bg-error-500/10 dark:text-error-400">
          <HiOutlineExclamationTriangle size={16} className="mt-0.5 shrink-0" />
