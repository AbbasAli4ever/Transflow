...
function ChangeStatusModal({
  ...
  onClose: () => void;
  onSaved: (updated: ApiCustomer) => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (apiErr.statusCode === 409) {
        ...
        setError(apiErr.message ?? "Failed to save customer.");
      }
    } finally {
      setSaving(false);
    }
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between border-b border-gray-100 px-6 py-5 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Edit Customer</h2>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Update customer details.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto space-y-4 px-6 py-5">
            ...
            <div>
              ...
              <input required className={inputClass} placeholder="Customer name" value={form.name}
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-700">
                {["Date", "Document #", "Type", "Description", "Debit (AR Decrease)", "Credit (AR Increase)", "Running Balance"].map((col) => (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60">
              {statement.entries.map((entry, i) => (
                <tr key={i} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/40">
                  ...
                  <td className={`py-3.5 pr-4 text-sm font-semibold whitespace-nowrap ${entry.credit > 0 ? "text-error-600" : "text-success-600"}`}>
                    {entry.type}
                  </td>
...
function OpenDocumentsTab({ openDocs, customerId }: { openDocs: CustomerOpenDocumentsResponse | null; customerId: string }) {
    ...
    <div>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
            Unpaid and partially-paid sales invoices with outstanding receivables.
          </p>
        </div>
        <button className="flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
          <HiOutlineArrowDownTray size={15} />
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  {["Document #", "Date", "Total Amount", "Received Amount", "Outstanding", "Days Outstanding", "Action"].map((col) => (
                      ...
                      <td className="py-3.5">
                        <button
                          ...
                          onClick={() => {
                            ...
                            window.location.href = "/transactions/customer-payment";
                          }}
    ...
    <div>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
            All transactions linked to this customer.
          </p>
        </div>
      </div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <option value="CUSTOMER_RETURN">Return</option>
        </select>
        <select
          value={statusFilter}
...
export default function CustomerDetailClient({ id }: { id: string }) {
  ...
  const [openDocs, setOpenDocs] = useState<CustomerOpenDocumentsResponse | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const [cust, bal, docs] = await Promise.all([
          getCustomer(id),
          getCustomerBalance(id),
          getCustomerOpenDocuments(id),
        ]);
        setCustomer(cust);
        setBalance(bal);
        setOpenDocs(docs);
    ...
    <div className="mx-auto w-full max-w-full">
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard
          ...
          value={balance ? formatPKR(balance.totalSales) : "—"}
          sub="Sum of all posted sales"
        />
        <StatCard
          label="Total Received (Payments + Returns)"
          value={balance ? formatPKR(balance.totalPayments + balance.totalReturns) : "—"}
          sub="Payments and returns applied"
        />
        <StatCard
          label="Current Balance"
          value={balance ? formatPKR(balance.currentBalance) : "—"}
          sub="Outstanding balance from customer"
          valueSuffix={
            balance && (
              <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${balanceLabelColor}`}>
                {balanceLabel}
              </span>
            )
          }
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="p-5">
          {activeTab === "ledger" && <LedgerTab customerId={customer.id} />}
          {activeTab === "openDocuments" && <OpenDocumentsTab openDocs={openDocs} customerId={customer.id} />}
          {activeTab === "transactions" && <TransactionsTab customerId={customer.id} />}
        </div>
      </div>
